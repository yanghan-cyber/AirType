...
2. **下载 ASR 模型**
   ...
   从 [Hugging Face](https://huggingface.co/Qwen/Qwen3-ASR-0.6B) 下载模型文件，放到 `models/` 目录：
   ```
   models/
   └── Qwen3-ASR-0.6B/
       ├── model.safetensors
...
3. **启动 ASR 后端**
   ...
   使用 [llama.cpp](https://github.com/ggerganov/llama.cpp) 或兼容的 OpenAI API 服务：
   ```bash
   # 示例：使用 llama-server
   llama-server -m models/Qwen3-ASR-0.6B/model.gguf --port 8178
   ```
...
**Q: 支持哪些 ASR 模型？**
A: 支持任何 OpenAI 兼容的 ASR API。推荐使用 Qwen3-ASR-0.6B 或 Qwen3-ASR-1.7B。
